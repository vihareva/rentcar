                    ...
                    <Col lg={16} sm={24}>
                        ...
                        {bookings.filter(o => o.user == user._id).map((booking) => {
                            return <Row gutter={16}
                                        className={moment(Date.now()).isAfter(moment(booking.bookedTimeSlots.to)) ? "bs1 mt-3 text-left nonactive" : "bs1 mt-3 text-left"}>
                                <Col lg={6} sm={24}>
                                ...
                                <Col lg={6} sm={24}
                                    ...
                                    <button disabled={moment(Date.now()).isAfter(moment(booking.bookedTimeSlots.to))}
                                            className="btn1 mr-1">
                                    ...
                                    {/*поищи отзыв на это бронирование, если его нет=>можно оставить отзыв */}
                                    {reviews.filter(r => r.booking === booking._id).length === 0 &&
                                    <button
                                        // disabled={moment(Date.now()).isBefore(moment(booking.bookedTimeSlots.to))}
                                        className="btn1">
                                        <a href={`/addreview/${booking._id}`}>Add review</a>
                                    </button>}
                                </Col>
                            </Row>;
                        })}
